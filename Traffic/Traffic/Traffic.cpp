...
int CONST SPEED = 10; // Høy = Rask, Lav = Treg.   Hvis den er for høy vil bilene ikke stoppe på rødt lys.
int RATE_V = 100; // 100 = 1 car per second at average
int RATE_H = 100; 
bool anythingChanged = false; //denne brukes i timer for å sjekke om noe har endret seg, hvis ja så lages en InvalidateRect
RECT textBox;

/*
TODO: 
BAD BUGS
- Fiks at biler med høy fart ikke stopper på rødt lys
- Fiks flickering
*/
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_TIMER:
	{
		...
		*/
		...
		if (time == 3600) //reset av time sånn at den aldri blir for stor
		{
			time = 0;
		}
...
VOID drawTrafficlight(HDC hdc, int left, int top, double mult, int state)
{
	...
	//fargelegger øverste lys enten rødt eller grått
	if (state == 1 || state == 2)
	{
	...
	//fargelegger midterste lys enten gult eller grått
	if (state == 2 || state == 4) 
	{
	...
	//fargelegger nederste lys enten grønt eller grått
	if (state == 3)
	{
